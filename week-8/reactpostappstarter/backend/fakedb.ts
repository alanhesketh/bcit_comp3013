...
export const addPost = (post: any) => {
  ...
  post.data.id = (posts.reduce((acc, post) => acc = acc > post.id ? acc : post.id, 0)) + 1;
  post.data.userId = post.userId;
  posts.push(post.data);
};

export const updatePost = (post: any) => {
  //Find the index of the post that needs to be updated.
  const objIndex = posts.findIndex((obj => obj.id == post.id));
  posts[objIndex].title = post.title;
  posts[objIndex].content = post.content;
  posts[objIndex].category = post.category;
  posts[objIndex].image = post.image;
};
